	...
	const Eigen::Vector3f& camWorldPos)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			Eigen::Vector3f normP = (b0 * t.norms[0] / depth0 + b1 * t.norms[1] / depth1 + b2 * t.norms[2] / depth2).normalized();
			...
			// incorrect depths.
			float depth = depthP * (b0 * t.screen[0].z() / depth0 + b1 * t.screen[1].z() / depth1 + b2 * t.screen[2].z() / depth2);
			// *** END YOUR CODE ***
