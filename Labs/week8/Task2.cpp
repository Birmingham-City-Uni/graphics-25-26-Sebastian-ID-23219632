	...
	const Eigen::Vector3f& camWorldPos)
{
	...
	for(int x = minX; x <= maxX; ++x) 
		for (int y = minY; y <= maxY; ++y) {
			...
			float depth0 = 1.0f / -t.cam[0].z(), depth1 = 1.0f / -t.cam[1].z(), depth2 = 1.0f / -t.cam[2].z();
			...
			// incorrect depths.
			float depth = (t.screen[0].z() * b0 * depth0 + t.screen[1].z() * b1 * depth1 + t.screen[2].z() * b2 * depth2) * depthP;
			// *** END YOUR CODE ***
