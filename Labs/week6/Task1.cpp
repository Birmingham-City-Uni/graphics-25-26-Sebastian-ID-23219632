...
Eigen::Matrix4f projectionMatrix(int height, int width, float horzFov = 70.f*M_PI/180.f, float zFar = 10.f, float zNear = 0.1f)
{
	...
	float vertFov = 2.0f * atanf(tanf(horzFov * 0.5f) * (float(height) / float(width)));
	// Now construct the matrix.
	float sx = 1.0f / tanf(horzFov * 0.5f);
	float sy = 1.0f / tanf(vertFov * 0.5f);
	...
	int width, int height)
{
	for (int i = 0; i < mesh.vFaces.size(); ++i) {
		...
		t.screen[0] = Eigen::Vector3f(vClip0.x() * width * 0.5f + width * 0.5f, -vClip0.y() * height * 0.5f + height * 0.5f, vClip0.z());
		t.screen[1] = Eigen::Vector3f(vClip1.x() * width * 0.5f + width * 0.5f, -vClip1.y() * height * 0.5f + height * 0.5f, vClip1.z());
		t.screen[2] = Eigen::Vector3f(vClip2.x() * width * 0.5f + width * 0.5f, -vClip2.y() * height * 0.5f + height * 0.5f, vClip2.z());
		// *** END YOUR CODE ***
